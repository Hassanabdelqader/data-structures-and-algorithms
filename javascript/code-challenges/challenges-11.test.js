...
const totalSum = (input) => {
  // Solution code here...
    ...
    sum +=input.reduce((acc,element) =>{
      
    acc+= element.reduce((acc,item)=>{
      acc +=item;
      return acc;
    },sum);
      return acc;
...
// Solution code here
  let arr = data.map(element=>{
    return (element.gender === 'male' || element.gender === 
  'female') ? element.name + ' and ' :'';
  });
