...
const charCode = (arr) => {
  const newArr = arr.map(v => v.charCodeAt(0));
  return newArr;
};
...
const evenOdd = (arr) => {
  ...
  const newArr = arr.map(v =>{
    return (!Number.isInteger(v))? 'N/A':(v%2===0)? 'even':'odd';
  });
  return newArr;
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  const newArr = arr.map(v=>v.ability.name);
  return newArr;
};
...
const extractStats = (arr) => {
  // Solution code here...
  const newArr = arr.map(v =>({'name' : v.stat.name ,'total' : v.baseStat+v.effort}));
  return newArr;
};
