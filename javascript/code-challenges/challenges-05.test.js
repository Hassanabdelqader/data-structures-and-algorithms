...
const addPurchases = (arr) => {
  // Solution code here...
  const sumPurchase = arr.reduce((acc,item) => acc += item.purchasePrice ,0);
  return sumPurchase; 
};
...
const countNumberOfElements = (arr) => {
  ...
  const sumIndex = arr.reduce((acc,item) => acc+=1 ,0);
...
const reversedString = (str) => {
  // Solution code here...
  let arr = str.split('');
  let newStr =arr.reduce((acc,value) =>{
    acc.unshift(value);
    return acc;
  },[]);
  return newStr.join('');
...
const countNumberOfChildren = (arr) => {
  ...
  const sum = arr.reduce((acc,value)=>{
    let temp= value.children?.length;
...
const calculateAverage = (arr) => {
  // Solution code here...
  const average = arr.reduce((acc,value,idx,arr) =>{
    acc.count++ ;
    acc.sum+=value;
